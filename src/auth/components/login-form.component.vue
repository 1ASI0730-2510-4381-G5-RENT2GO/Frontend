<template>
  <form
      @submit.prevent="onSubmit"
      class="max-w-md w-full animate-slide-in shadow-lg px-8 py-10
         bg-white dark:bg-gray-800
         rounded-2xl border border-gray-200 dark:border-gray-700
         transition-all duration-500">
    <div class="flex flex-col items-center justify-center mb-8">
      <img src="@/assets/logo.svg" alt="Logo" class="h-16 mb-4 drop-shadow-md select-none" draggable="false" />
      <h2 class="text-2xl font-extrabold text-gray-800 dark:text-white mb-2 transition-colors">
        {{ $t('auth.login_title') }}
      </h2>
      <p class="text-gray-600 dark:text-gray-300 text-sm text-center transition-colors">
        {{ $t('auth.login_subtitle') }}
      </p>
    </div>

    <div class="space-y-4 mb-6">
      <div class="relative">
        <div class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400">
          <i class="pi pi-envelope"></i>
        </div>
        <input
            v-model="email"
            type="email"
            class="w-full rounded-xl px-4 py-3 pl-10 border border-gray-300 dark:border-gray-600
              bg-white dark:bg-gray-700
              text-gray-800 dark:text-gray-200
              focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-600
              focus:border-primary-500 dark:focus:border-primary-600
              outline-none transition-colors duration-300
              placeholder:text-gray-400 dark:placeholder:text-gray-500"
            :placeholder="$t('auth.email_placeholder')"
            required
            autocomplete="username"
        />
      </div>

      <div class="relative">
        <div class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400">
          <i class="pi pi-lock"></i>
        </div>
        <input
            v-model="password"
            :type="showPassword ? 'text' : 'password'"
            class="w-full rounded-xl px-4 py-3 pl-10 pr-10 border border-gray-300 dark:border-gray-600
              bg-white dark:bg-gray-700
              text-gray-800 dark:text-gray-200
              focus:ring-2 focus:ring-primary-500 dark:focus:ring-primary-600
              focus:border-primary-500 dark:focus:border-primary-600
              outline-none transition-colors duration-300
              placeholder:text-gray-400 dark:placeholder:text-gray-500"
            :placeholder="$t('auth.password_placeholder')"
            required
            autocomplete="current-password"
        />
        <button
            type="button"
            @click="showPassword = !showPassword"
            class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
        >
          <i :class="showPassword ? 'pi pi-eye-slash' : 'pi pi-eye'"></i>
        </button>
      </div>
    </div>

    <div class="flex justify-end mb-6">
      <router-link
          to="/forgot-password"
          class="text-sm text-primary-600 dark:text-primary-400 hover:underline transition-colors"
      >
        {{ $t('auth.forgot_password') }}
      </router-link>
    </div>

    <button
        type="submit"
        :disabled="loading"
        class="w-full rounded-xl py-3 font-bold
          bg-primary-600 dark:bg-primary-500
          text-white shadow-md hover:shadow-lg
          hover:bg-primary-700 dark:hover:bg-primary-600
          transition-all duration-300
          focus:ring-2 focus:ring-primary-500/50 dark:focus:ring-primary-500/60
          disabled:opacity-70 disabled:cursor-not-allowed"
    >
      <span v-if="!loading" class="flex items-center justify-center">
        {{ $t('auth.login_button') }}
      </span>
      <span v-else class="flex items-center justify-center">
        <i class="pi pi-spin pi-spinner mr-2"></i>
        {{ $t('auth.loading') }}
      </span>
    </button>

    <transition name="fade-slide" mode="out-in">
      <div
          v-if="error"
          key="error"
          class="mt-4 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-300 font-medium rounded-xl py-3 px-4 flex items-center"
      >
        <i class="pi pi-exclamation-circle mr-2"></i>
        {{ error }}
      </div>
    </transition>

    <div class="flex items-center my-6">
      <div class="flex-grow h-px bg-gray-200 dark:bg-gray-700"></div>
      <span class="px-4 text-sm text-gray-400 dark:text-gray-500">{{ $t('auth.continue_with') }}</span>
      <div class="flex-grow h-px bg-gray-200 dark:bg-gray-700"></div>
    </div>

    <div class="grid grid-cols-3 gap-3 mb-6">

      <button
          type="button"
          @click="loginWithSocial('google')"
          :disabled="loading"
          class="flex justify-center items-center py-2.5 px-4 rounded-xl
            bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600
            text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600
            shadow-sm hover:shadow transition-all duration-300
            disabled:opacity-50 disabled:cursor-not-allowed"
      >
        <svg class="w-5 h-5" viewBox="0 0 24 24">
          <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
          <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
          <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
          <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
        </svg>
      </button>

      <button
          type="button"
          @click="loginWithSocial('facebook')"
          :disabled="loading"
          class="flex justify-center items-center py-2.5 px-4 rounded-xl
            bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600
            text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600
            shadow-sm hover:shadow transition-all duration-300
            disabled:opacity-50 disabled:cursor-not-allowed"
      >
        <svg class="w-5 h-5" viewBox="0 0 24 24">
          <path fill="#1877F2" d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
        </svg>
      </button>

      <button
          type="button"
          @click="loginWithSocial('twitter')"
          :disabled="loading"
          class="flex justify-center items-center py-2.5 px-4 rounded-xl
            bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600
            text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600
            shadow-sm hover:shadow transition-all duration-300
            disabled:opacity-50 disabled:cursor-not-allowed"
      >
        <svg class="w-5 h-5" viewBox="0 0 24 24">
          <path fill="#1DA1F2" d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
        </svg>
      </button>
    </div>

    <div class="text-sm text-center text-gray-600 dark:text-gray-400 transition-colors">
      {{ $t('auth.no_account') }}
      <router-link
          to="/register"
          class="font-medium text-primary-600 dark:text-primary-400 hover:underline transition-colors"
      >
        {{ $t('auth.register') }}
      </router-link>
    </div>
  </form>
</template>

<script setup>
import { ref } from "vue";
import { login, loginWithProvider, getStoredUser } from "../services/auth.service.js";
import { useRouter } from 'vue-router';
import { useI18n } from 'vue-i18n';
import { useToast } from "primevue/usetoast";
import { useAuthStore } from "@/auth/stores/authStore.js";

const email = ref("");
const password = ref("");
const error = ref("");
const loading = ref(false);
const showPassword = ref(false);
const router = useRouter();
const { t } = useI18n();
const toast = useToast();

async function onSubmit() {
  error.value = "";
  loading.value = true;

  try {
    if (!email.value || !password.value) {
      error.value = t('auth.required_fields');
      loading.value = false;
      return;
    }

    const auth = useAuthStore();

    try {
      const credentials = {
        email: email.value,
        password: password.value
      };
      
      console.log("Enviando credenciales:", credentials);

      await auth.login(credentials);

      toast.add({
        severity: 'success',
        summary: t('auth.success'),
        detail: t('auth.login_success'),
        life: 3000
      });

      const user = auth.user;
      console.log("Usuario autenticado:", user);

      if (user && user.role) {
        const path = getRedirectPathByRole(user.role);
        console.log(`Redirigiendo a: ${path}`);
        router.push(path);
      } else {
        console.error("No se encontró rol de usuario después del login");
        router.push('/client/home');
      }
    } catch (err) {
      console.error('Error durante login en el store:', err);
      error.value = err.message || t('auth.error_credentials');
    }
  } catch (err) {
    console.error('Login Error general:', err);
    error.value = err.message || t('auth.error_credentials');
  } finally {
    loading.value = false;
  }
}

async function loginWithSocial(provider) {
  loading.value = true;
  error.value = "";

  try {
    const auth = useAuthStore();
    const response = await auth.loginWithOAuth(provider);

    toast.add({
      severity: 'success',
      summary: t('auth.success'),
      detail: t('auth.login_success'),
      life: 3000
    });

    const user = auth.user;
    console.log("Usuario OAuth autenticado:", user);

    if (user && user.role) {
      router.push(getRedirectPathByRole(user.role));
    } else {
      router.push('/client/home');
    }
  } catch (err) {
    console.error(`${provider} Login Error:`, err);
    error.value = err.message || t('auth.error_social_login');
  } finally {
    loading.value = false;
  }
}

function getRedirectPathByRole(role) {
  console.log(`Determinando ruta para rol: ${role}`);

  switch (role) {
    case 'admin':
      return '/admin/dashboard';
    case 'provider':
      return '/provider/dashboard';
    case 'client':
      return '/client/home';
    default:
      console.warn(`Rol desconocido: ${role}, redirigiendo a ruta por defecto`);
      return '/client/home';
  }
}
</script>

